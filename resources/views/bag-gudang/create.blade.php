...
@section('title', 'Tambah Bag Gudang - Sistem Manajemen Gudang')

@section('content')
    <div class="page-header">
        <h1>Tambah Bag Gudang</h1>
        <p>Tambahkan data bag gudang baru</p>
    </div>

    <div class="form-container">
        <form action="{{ route('bag-gudang.store') }}" method="POST">
            @csrf
            <div class="form-group">
                <label for="kode_bag_gudang">ID Bag Gudang</label>
                <input type="text" name="kode_bag_gudang" id="kode_bag_gudang" value="{{ old('kode_bag_gudang') }}" placeholder="Contoh: BG006" required>
            ...
            <div class="form-group">
                ...
                <input type="text" name="nama" id="nama" value="{{ old('nama') }}" placeholder="Nama Bag Gudang" required>
                @error('nama')
                    <div style="color: red; font-size: 12px; margin-top: 5px;">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="form-group">
                ...
                <textarea name="alamat" id="alamat" rows="3" placeholder="Alamat lengkap" required>{{ old('alamat') }}</textarea>
                @error('alamat')
                    <div style="color: red; font-size: 12px; margin-top: 5px;">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="form-group">
                ...
                <input type="text" name="no_telepon" id="no_telepon" value="{{ old('no_telepon') }}" placeholder="Nomor telepon" required>
                @error('no_telepon')
                    <div style="color: red; font-size: 12px; margin-top: 5px;">{{ $message }}</div>
                @enderror
            </div>
