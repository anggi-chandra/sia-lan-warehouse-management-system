...
@section('content')
    <div class="page-header">
        <h1>Tambah Admin</h1>
        <p>Tambahkan data admin baru</p>
    </div>

    <div class="form-container">
        <form action="{{ route('bag-admin.store') }}" method="POST">
            @csrf
            <div class="form-group">
                <label for="kode_admin">ID Admin</label>
                <input type="text" id="kode_admin" name="kode_admin" value="{{ old('kode_admin') }}" placeholder="Contoh: ADM005" required>
                @error('kode_admin')
            ...
            <div class="form-group">
                ...
                <input type="text" id="nama_admin" name="nama_admin" value="{{ old('nama_admin') }}" placeholder="Nama Admin" required>
                @error('nama_admin')
                    <div style="color: red; font-size: 12px; margin-top: 5px;">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="form-group">
                ...
                <textarea id="alamat" name="alamat" rows="3" placeholder="Alamat lengkap" required>{{ old('alamat') }}</textarea>
                @error('alamat')
                    <div style="color: red; font-size: 12px; margin-top: 5px;">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="form-group">
                ...
                <input type="text" id="no_telepon" name="no_telepon" value="{{ old('no_telepon') }}" placeholder="Nomor telepon" required>
                @error('no_telepon')
                    <div style="color: red; font-size: 12px; margin-top: 5px;">{{ $message }}</div>
                @enderror
            </div>
