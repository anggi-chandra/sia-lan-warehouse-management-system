...
@section('content')
    ...
    <div class="form-container">
        <form action="{{ route('bag-admin.update', $bagAdmin->id) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="form-group">
                <label for="kode_admin">ID Admin</label>
                <input type="text" id="kode_admin" name="kode_admin" value="{{ old('kode_admin', $bagAdmin->kode_admin) }}" placeholder="Contoh: ADM005" required>
                @error('kode_admin')
            ...
            <div class="form-group">
                ...
                <input type="text" id="nama_admin" name="nama_admin" value="{{ old('nama_admin', $bagAdmin->nama_admin) }}" placeholder="Nama Admin" required>
                @error('nama_admin')
                    <div style="color: red; font-size: 12px; margin-top: 5px;">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="form-group">
                ...
                <input type="email" id="email" name="email" value="{{ old('email', $bagAdmin->email) }}" placeholder="Email" required>
                @error('email')
                    <div style="color: red; font-size: 12px; margin-top: 5px;">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="form-group">
                ...
                <textarea id="alamat" name="alamat" rows="3" placeholder="Alamat lengkap" required>{{ old('alamat', $bagAdmin->alamat) }}</textarea>
                @error('alamat')
                    <div style="color: red; font-size: 12px; margin-top: 5px;">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="form-group">
                ...
                <input type="text" id="no_telepon" name="no_telepon" value="{{ old('no_telepon', $bagAdmin->no_telepon) }}" placeholder="Nomor telepon" required>
                @error('no_telepon')
                    <div style="color: red; font-size: 12px; margin-top: 5px;">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="form-group">
                ...
                <div style="position: relative;">
                    <input type="password" id="password" name="password" placeholder="Password baru" style="padding-right: 40px;">
            ...
            <div class="form-actions">
                <a href="{{ route('bag-admin.index') }}" class="btn-cancel">Batal</a>
